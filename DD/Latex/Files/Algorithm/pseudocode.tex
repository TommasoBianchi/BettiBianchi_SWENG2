...
\caption{INSERT MEETING}
	...
	\Function{insert\_meeting}{$new\_meeting$, $user$}
		...
		\If{$before\_meeting$ is not NULL \textbf{and} $arriving\_travel.duration$ > $new\_meeting.start\_date$ - $before\_meeting.leaving\_travel.end\_time$}
			\State $arriving\_travel$ := \Call{best\_travel}{$before\_meeting.leaving\_travel.arrival$, $new\_meeting.location$, $user$}
		...
		\If{$after\_meeting$ is not NULL \textbf{and} $leaving\_travel$.$duration$ > $after\_meeting$.$arriving\_travel$.$start\_time$ - $new\_meeting$.$end\_date$}
			\State $leaving\_travel$ := \Call{best\_travel}{$new\_meeting.location$, $after\_meeting.arriving\_travel.departure$, $user$}
			\If{$leaving\_travel.duration$ > $after\_meeting.arriving\_travel.start\_time$ - $new\_meeting.end\_date$}
				\State $leaving\_travel$ := \Call{best\_travel}{$new\_meeting.location$, $before\_meeting.location$, $user$}
...
\caption{BEST TRAVEL}
	...
	\Function{best\_travel}{$from$, $to$, $user$}
		...
		\ForAll{$travel\_mean$ \textbf{in} $user.preference\_list$}
			...
			\ForAll{$constraint$ \textbf{in} $user.constraints.filter(travel\_mean)$}
